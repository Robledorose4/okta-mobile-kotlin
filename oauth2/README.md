...
- When the ID-JAG itself expires (`idJag.isExpired(clock)`), call `start()` again with the original subject assertion.
...
flow.exchange(SubjectAssertion.idToken(idToken))
    .thenAccept(resourceToken -> {
        String accessToken = resourceToken.getAccessToken();
    });
flow.close();
```

For deep target-client customization — a shared executor, a custom cache — build the target client directly with `OAuth2ClientBuilder` and wrap it instead of using the chaining builder:
...
CrossAppAccessTarget target = CrossAppAccessTarget.wrapping(targetClient, null, null);
```
